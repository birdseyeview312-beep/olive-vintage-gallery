...
async function loadDashboard(){
  ...
  const [productsRes,eventsRes,lotsRes,regsRes,webOrdersRes,privateSessionsRes,privateRequestsRes]=await Promise.all([
  ...
  Promise.allSettled([functionGet("paypal-settings"),functionGet("mux-settings")]).then(results=>{const p=results[0].status==="fulfilled"?results[0].value:null;const m=results[1].status==="fulfilled"?results[1].value:null;e.paymentCardStatus.textContent=p?.configured?`PayPal connected · ${p.mode==="live"?"LIVE":"Sandbox"}`:"PayPal not connected";e.videoCardStatus.textContent=m?.configured?"Mux connected":"Mux not connected";});
}
