...
async function loadDashboard(){
  ...
  Promise.allSettled([functionGet("square-settings"),functionGet("mux-settings")]).then(results=>{const p=results[0].status==="fulfilled"?results[0].value:null;const m=results[1].status==="fulfilled"?results[1].value:null;e.paymentCardStatus.textContent=p?.configured?`Square connected · ${p.mode==="live"?"Live":"Sandbox"}`:"Square not connected";e.videoCardStatus.textContent=m?.configured?"Mux connected":"Mux not connected";});
}
