...
async function functionGet(slug){
  const {data:{session:s}}=await supabase.auth.getSession();
  if(!s?.access_token) throw new Error("Owner session expired");
  const r=await fetch(`${SUPABASE_URL}/functions/v1/${slug}`,{headers:{Authorization:`Bearer ${s.access_token}`,apikey:SUPABASE_ANON_KEY}});
...
async function loadDashboard(){
  ...
  const [productsRes,eventsRes,lotsRes,regsRes,webOrdersRes]=await Promise.all([
  ...
  const products=productsRes.data||[],events=eventsRes.data||[],lots=lotsRes.data||[];
  const available=products.filter(x=>x.status==="available").length;
  const drafts=products.filter(x=>x.status==="draft").length;
  const sold=products.filter(x=>x.status==="sold").length;
  const activeAuctions=events.filter(x=>["scheduled","live"].includes(x.status)).length;
  const liveLots=lots.filter(x=>x.status==="live").length;
  const soldLots=lots.filter(x=>x.status==="sold").length;
  const webOrders=webOrdersRes.data||[];
  const paidWebOrders=webOrders.filter(x=>x.status==="paid").length;
  const unfulfilledWebOrders=webOrders.filter(x=>x.status==="paid"&&x.fulfillment_status!=="completed").length;
  e.overviewStats.innerHTML=`
    <div class="overview-stat"><span>Total inventory</span><strong>${productsRes.count??products.length}</strong><small>${available} available · ${drafts} draft</small></div>
    <div class="overview-stat"><span>Auction events</span><strong>${eventsRes.count??events.length}</strong><small>${activeAuctions} scheduled/live</small></div>
    <div class="overview-stat"><span>Registered bidders</span><strong>${regsRes.count??0}</strong><small>Across all auction events</small></div>
    <div class="overview-stat"><span>Completed sales</span><strong>${sold}</strong><small>${paidWebOrders} web · ${soldLots} auction</small></div>`;
  ...
  Promise.allSettled([functionGet("paypal-settings"),functionGet("mux-settings")]).then(results=>{
    const p=results[0].status==="fulfilled"?results[0].value:null;
    const m=results[1].status==="fulfilled"?results[1].value:null;
    e.paymentCardStatus.textContent=p?.configured?`PayPal connected · ${p.mode==="live"?"LIVE":"Sandbox"}`:"PayPal not connected";
    e.videoCardStatus.textContent=m?.configured?"Mux connected":"Mux not connected";
  });
}
function render(){
  const ok=isAdmin();
  e.loginPanel.classList.toggle("hidden",ok);
  e.dashboardApp.classList.toggle("hidden",!ok);
  e.signOutBtn.classList.toggle("hidden",!session);
  e.sessionEmail.textContent=session?.user?.email||"";
  if(session&&!ok)e.loginMessage.textContent="This account does not have Olive Vintage owner access.";
  if(ok){e.loginMessage.textContent="";loadDashboard();}
}
